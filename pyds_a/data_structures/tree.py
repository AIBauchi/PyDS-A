...
in a hierarchical manner in the form of parent and child node, where a child node can be a parent node for other nodes.

The Tree class provides methods for adding child nodes(data) to the tree, checking how deep a node is in
the tree, and to print the entire tree structure in a hierarchical manner such  that the hierarchy is visible.
...
class Tree:
  """
        ...
        A tree is a non-linear data structure in which the members are connected using references in a hierarchical
        manner in the form of parent and child node, where a child node  can be a parent node for other nodes.
